...
export class GenerarComponent implements OnInit {
    ...
    constructor(private http: HttpClient, private modalService: NgbModal, private renderer: Renderer2, private router: Router, private auth: AuthService) {
    ...
    ngOnInit() {
        ...
        this.http.get(`${backend_url}almacen/prestamo/generar/data`)
            .subscribe(
                res => {
    ...
    agregarProducto() {
        ...
        const producto = this.data.productos.find(producto => producto.sku == this.producto.sku);
        ...
        this.http.get(`${backend_url}venta/venta/crear/producto/existencia/${this.producto.sku}/${this.data.almacen_salida}/${this.producto.cantidad}`)
            .subscribe(
                res => {
                    if (res['code'] != 200) {
    ...
    buscarProducto() {
        ...
        this.http.get(`${backend_url_erp}api/adminpro/producto/Consulta/Productos/SKU/${bd}/${this.producto.codigo_text}`)
            .subscribe(
                res => {
                    ...
                    this.http.get(`${backend_url_erp}api/adminpro/producto/Consulta/Productos/Descripcion/${bd}/${this.producto.codigo_text}`)
                        .subscribe(
                            res => {
                                if (Object.values(res).length == 0) {
                                    swal("", "No se encontró el producto, favor de revisar la información e intentar de nuevo.", "error");
    ...
    eliminarProducto(codigo) {
        const index = this.data.productos.findIndex(producto => producto.sku == codigo);
    ...
    agregarSeries(modal, codigo) {
        ...
        const producto = this.data.productos.find(producto => producto.sku == codigo);
    ...
    eliminarSerie(serie) {
        const index = this.data.series.findIndex(serie_ip => serie_ip == serie);

        this.data.series.splice(index, 1);

        const producto = this.data.productos.find(producto => producto.sku == this.data.producto_serie);
    ...
    confirmarSeries() {
        ...
        this.http.post(`${backend_url}almacen/movimiento/crear/confirmar`, form_data)
            .subscribe(
                res => {
                    if (res['code'] != 200) {
                        swal("", res['message'], "error");
    ...
    async serieRepetida(serie) {
        try {
            ...
            const repetida = this.data.productos.find(producto => producto.series.find(serie_ip => serie_ip == serie));
    ...
    tokenAuthy(modal) {
        const producto = this.data.productos.find(producto => producto.serie && (producto.series.length < 1));
    ...
    crearDocumento(modal) {
        ...
        this.http.post(`${backend_url}almacen/prestamo/generar`, form_data)
            .subscribe(
                res => {
                    if (res['code'] != 400) {
                        swal({
    ...
    cambiarEmpresa() {
        const empresa = this.empresas.find(empresa => empresa.id == this.data.empresa);
