...
export class RegresarComponent implements OnInit {
    ...
    ngOnInit() {
        this.http.get(`${backend_url}almacen/prestamo/regresar/data`).subscribe(
            (res) => {
    ...
    eliminarSerie(serie) {
        const index = this.detalle.series.findIndex(
        ...
        const producto = this.detalle.productos.find(
            (producto) => producto.sku == this.detalle.producto_serie
        );
    ...
    confirmarSeries() {
        ...
        const serie = this.detalle.series.find(
            (serie) => $.inArray(serie, producto.series_anteriores) === -1
        );
        ...
        if (serie) {
            swal(
                ...
                'La serie ' +
                    serie +
                    ' no pertecene a este prestamo, favor de verficiar',
                'error'
            );
    ...
    guardarDocumento() {
        const producto = this.detalle.productos.find(
            (producto) =>
                producto.serie &&
                producto.series_anteriores.length != producto.series.length
        );

        if (producto) {
            swal(
                '',
                'La cantidad de series registradas no concuerda con la cantidad requerida en el producto ' +
                    producto.sku +
                    ', faltan: ' +
                    (producto.series_anteriores.length -
        ...
        this.http
            .post(`${backend_url}almacen/prestamo/regresar/guardar`, form_data)
            .subscribe(
                (res) => {
                    if (res['code'] != 400) {
                        swal({
