...
@Component({
    ...
    styleUrls: ['./indemnizacion.component.scss']
})
export class IndemnizacionComponent implements OnInit {
    ...
    constructor(private http: HttpClient, private chRef: ChangeDetectorRef, private modalService: NgbModal) {
        const table_producto: any = $("#soporte_garantia_devolucion_devolucion_indemnizacion");

        this.datatable = table_producto.DataTable();
    }

    ngOnInit() {
        this.http.get(`${backend_url}soporte/garantia-devolucion/devolucion/indemnizacion/data`)
            .subscribe(
                res => {
                    ...
                    const table: any = $("#soporte_garantia_devolucion_devolucion_indemnizacion");
    ...
    guardarDocumento() {
        ...
        this.http.post(`${backend_url}soporte/garantia-devolucion/devolucion/indemnizacion/guardar`, form_data)
            .subscribe(
                res => {
                    swal({
                        title: "",
                    ...
                    if (res['code'] == 200) {
                        if (this.final_data.terminar) {
                            const index = this.ventas.find(venta => venta.id == this.final_data.documento);
    ...
    verArchivo(id_dropbox) {
        this.http
            .post<any>(
                `${backend_url}/dropbox/get-link`, // Tu endpoint backend centralizado
                { path: id_dropbox }
            )
            .subscribe(
                res => {
