...
export class HistorialComponent implements OnInit {
    ...
    detalleVenta(modal, documento) {
        ...
        this.data.garantia.paqueteria_envio = venta.paqueteria_envio;
        this.data.garantia.guia_llegada = venta.guia_llegada;
        this.data.garantia.guia_envio = venta.guia_envio;
    ...
    guardarDocumento() {
        ...
        this.http
            .post(
                `${backend_url}soporte/garantia-devolucion/garantia/historial/guardar`,
                form_data
            )
            .subscribe(
                (res) => {
    ...
    descargarDocumento() {
        this.http
            .get(
                `${backend_url}soporte/garantia-devolucion/garantia/historial/documento/${this.final_data.documento_garantia}`
            )
            .subscribe(
                (res) => {
                    if (res['code'] == 200) {
    ...
    cargarDocumentos() {
        this.soporteService.getGarantiaHistorialData(this.busqueda).subscribe(
            (res: any) => {
